...
#
#dim(datamatrix)
#class(datamatrix$TCGA.05.4244.01A)
#
...
count <- count[ , colnames(count) %in% top.genes]
...
featurePlot(x = testData[, 1:dim(testData)[2]-1], 
            y = testData$class, 
            plot = "box",
            strip=strip.custom(par.strip.text=list(cex=.7)),
            scales = list(x = list(relation="free"), 
...
set.seed(123) 
class(trainData$hsa.miR.101.5p)
...
testData.t <- testData
testData.t$class <- ifelse(testData.t$class=="tumor", "normal", "tumor")
testData.t$class <- as.factor(testData.t$class)

# Predict on testData 
stack_predicteds <- predict(stack.glm, newdata=testData.t)
summary(testData.t$class)

head(stack_predicteds) 

confusionMatrix(reference = testData.t$class, data = stack_predicteds, mode='everything', positive='tumor')  
