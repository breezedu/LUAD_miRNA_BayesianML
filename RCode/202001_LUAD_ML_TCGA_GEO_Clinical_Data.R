...
setwd("C:/Users/Jeff/OneDrive - QIAGEN GmbH/SVM_RFE_Prj/Jeff_Shuai/20200115_input_files/")
...
## setwd("C:/Users/Jeff/OneDrive - QIAGEN GmbH/SVM_RFE_Prj/0604")
...
luad_data2 <- apply(luad_data[, c(1:106)], 2, function(x) as.numeric(as.factor(x)))
luad_data2 <- as.data.frame(luad_data2)

luad_data2[1:5, 1:5]
rownames(luad_data2) <- row.names(luad_data)
class(luad_data2$hsa.miR.200b.3p)
...
luad_data$class <- ifelse(luad_data$class=="Solid Tissue Normal", "Tumor", "Normal")
summary(luad_data$class)
...
tiff("Plot_svmLinear_Combined_All_3.tif", width = 8, height = 8, units = 'in', res = 300)

rocobj_svmlinear <- roc(model_svmLinear$pred$obs, model_svmLinear$pred$yes, ci=TRUE,
                        plot=TRUE, 
                        legacy.axes=TRUE, percent=TRUE,
                        main="Combined all 3 Models svmLinear",
                        xlab="False Positive Percentage", 
                        ylab="True Postive Percentage", 
                        col="darkblue", lwd=5, 
                        print.auc=TRUE,
...
predicted_prob <- predict(model_svmLinear, testData, type = "prob")

str( predicted_prob )

rocobj_svmlinear.vliad <- roc( testData$class, predicted_prob[[2]] , ci=TRUE,
                              plot=TRUE, 
                              legacy.axes=TRUE, percent=TRUE,
                              main="svmLinear",
                              xlab="False Positive Percentage", 
...
legend("bottomright", 
       legend=c( "svmLinear Training", "svmLinear Validation"), 
       col=c( "darkblue", "red"), 
       lwd=4
)
...
predicted_prob <- predict(merged.models$svmLinear, testData, type = "prob")
...
### 
predicted2 <- predict(merged.models$svmLinear, testData)
