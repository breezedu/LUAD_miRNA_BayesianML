...
rocobj_models <- roc(merged.models$earth$pred$obs, 
                     merged.models$earth$pred$Tumor, 
                     ci=TRUE,
                     plot=TRUE, 
                     legacy.axes=TRUE, percent=TRUE, 
                     xlab="False Positive Percentage", 
...
stack_predicted.valid <- predict(stack.glm, newdata=validData) 
head(stack_predicted.valid) 

confusionMatrix(reference = validData$class, data = stack_predicted.valid, mode='everything', positive='Tumor') 


stack_predicted.apply <- predict(stack.glm, newdata=applicationData) 
head(stack_predicted.apply) 

confusionMatrix(reference = applicationData$class, data = stack_predicted.apply, mode='everything', positive='Tumor') 
